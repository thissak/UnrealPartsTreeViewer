...
bool FTreeViewUtils::HasChildWithImage(TSharedPtr<FPartTreeItem> Item)
{
    if (!Item.IsValid())
        return false;
...
bool FTreeViewUtils::FilterItemsByImage(TSharedPtr<FPartTreeItem> Item)
{
    if (!Item.IsValid())
        return false;
        
    // 현재 항목에 이미지가 있는지 확인
    bool bCurrentItemHasImage = FPartImageManager::Get().HasImage(Item->PartNo);
